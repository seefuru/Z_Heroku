...
from bot.helper.ext_utils.bot_utils import is_url, is_magnet, get_readable_time
...
class MirrorLeechListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        if BOT_PM and FORCE_BOT_PM and not self.isPrivate:
            ...
            sendMarkup(botpm, self.bot, self.message, buttons.build_menu(2))
            self.message.delete()
            reply_to = self.message.reply_to_message
        ...
        if self.isLeech:
            ...
            if not files:
                ...
                fmsg = '<b>Here are the links:</b>\n'
                if LEECH_LOG_LINK:
                    hp_ = f'\n<b>Check </b><a href=\"{LEECH_LOG_LINK}\"><b>Leech Dump</b></a>\n'
                else:
                    hp_ = ''
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    if len(fmsg.encode() + msg.encode()) > 4000:
                        if FORCE_BOT_PM is False:
                            upldmsg = sendMarkup(msg + fmsg + hp_, self.bot, self.message, buttons.build_menu(1))
                ...
                if LEECH_LOG:
                    for i in LEECH_LOG:
                        indexmsg = '<b>Here are the links:</b>\n'
                        for index, (link, name) in enumerate(files.items(), start=1):
                            indexmsg += f"{index}. <a href='{link}'>{name}</a>\n"
                            if len(indexmsg.encode() + msg.encode()) > 4000:
                                bot.sendMessage(chat_id=i, text=msg + indexmsg + hp_,
            ...
            if BOT_PM and self.message.chat.type != 'private':
                try:
                    bot.sendMessage(chat_id=self.user_id, text=f"{msg}{fmsg}" + hp_, allow_sending_without_reply=True, parse_mode='HTMl', disable_web_page_preview=True)
                except Exception as e:
                    LOGGER.warning(e)
                    return
