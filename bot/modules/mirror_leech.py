...
from bot import *
from bot.helper.ext_utils.bot_utils import is_url, is_magnet, is_mega_link, is_gdrive_link, get_content_type, get_user_task
...
from bot.helper.telegram_helper.message_utils import sendMessage, sendMarkup, auto_delete_message, forcesub
...
def _mirror_leech(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False):
    ...
    if BOT_PM and message.chat.type != 'private':
        try:
            ...
            buttons.buildbutton("Start", f"{botstart}")
            startwarn = f"<b>You didn't START the bot in DM</b>"
...
4. Commands that start with <b>qb</b> are ONLY for torrents.
    ...
    if is_gdrive_link(link):
        ...
        if MEGA_KEY is not None:
            ...
            sendMessage(f'<b>MEGA Not Available In This Bot.</b>', bot, message)
    elif isQbit:
